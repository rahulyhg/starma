<?php
require_once "header.php"; 
?>



 <div id="welcome">
    <?php flare_title("Welcome");?>
    <div id="header">
      Welcome to Starma.com. We're so glad you've joined our community!  Our site is still in development, so please <a href="mailto:contact@starm.com">contact us</a> if you encounter any problems.  Below are some of the ways you can get started.
    </div>
 
    <div id="profile_box_link" class="homepage_div">
      <a href="main.php?the_page=psel&the_left=nav1"></a>
    </div>
    <div id="compare_box_link" class="homepage_div">
      <a href="main.php?the_page=cosel&the_left=nav1&the_tier=1"></a>
    </div>
    <div id="horoscope_box_link" class="homepage_div">
      <a href="main.php?the_page=csel&the_left=nav1"></a>
    </div>
    <div id="celebrities_box_link" class="homepage_div">
      <a href="main.php?the_page=cosel&the_left=nav3"></a>
    </div>
 </div> 

