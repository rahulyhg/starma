...
//SUBMIT FROM LI------------------------------
	...
	$('.hl_li').click(function(){
		if ($(this).children().children('.pass_hl_id').val() !== 1) {
			$('#hl_prev').show();
			$('#hl_next').show();
		}
		if ($(this).children().children('.pass_hl_id').val() == 1) {
		...
		if ($(this).children().children('.pass_hl_id').val() == 12) {
			$('#hl_next').hide();
		}
		...
		$('#blurb').children().fadeOut(300);
			...
			var hl_data = {
				...
				'hl_id'            :   $(this).children().children('.pass_hl_id').val(),				
			...
			.done(function(data) {
				...
				if (data.errors) {
					...
					if(data.errors.hl_id) {
						$('#hl_id_err').text(data.hl_id).fadeIn(800);
					}
...
//PREV BUTTON---------------------------------------
	...
	$('#hl_prev').click(function(){
		...
		if ($('div.hl_tabs>ul>li.selected').children().children('.pass_hl_id').val() !== 7) {
		...
		if (prev.children().children('.pass_hl_id').val() !== 1) {
			$('#hl_prev').show();
			$('#hl_next').show();
		}
		if (prev.children().children('.pass_hl_id').val() == 1) {
		...
		if (prev.children().children('.pass_hl_id').val() == 12) {
			$('#hl_next').hide();
		}
		...
		$('#blurb').children().fadeOut(300);
			...
			var hl_data = {
				'rising_sign_id'   :   $('input[name=rising_sign_id]').val(),
				'hl_id'            :   prev.children().children('.pass_hl_id').val(),				
...
//NEXT BUTTON---------------------------------
	...
	$('#hl_next').click(function(){
		...
		if ($('div.hl_li>ul>li.selected').children().children('.pass_hl_id').val() !== 6) {
			var next = $('div.hl_tabs>ul>li.selected').next('li');
			//alert(prev.children().children('.pass_poi_id').val());
			//alert('hello');
		}
		if ($('div.hl_tabs>ul>li.selected').children().children('.pass_hl_id').val() == 6) {
		...
		if (next.children().children('.pass_hl_id').val() !== 1) {
			$('#hl_prev').show();
			$('#hl_next').show();
		}
		if (next.children().children('.pass_hl_id').val() == 1) {
		...
		if (next.children().children('.pass_hl_id').val() == 12) {
			$('#hl_next').hide();
		}
		...
		var hl_data = {
				'rising_sign_id'   :   $('input[name=rising_sign_id]').val(),
				'hl_id'            :   next.children().children('.pass_hl_id').val(),				
